...
paymentForm.include({
    ...
    _initPaymentContextObserver() {
        ...
        document.addEventListener('click', (event) => {
            console.log('[Stripe Badge] Click detected on:', event.target);
            ...
            if (amountButton) {
                ...
                setTimeout(() => {
                    console.log('[Stripe Badge] Checking amount after 150ms delay...');
                    ...
                    if (newAmount && newAmount !== this._lastObservedAmount) {
                        ...
                        console.log('[Stripe Badge] Amount unchanged:', newAmount);
                    }
                }, 150); // Increased delay for Bootstrap tab animation
            } else {
                console.log('[Stripe Badge] Click not on amount button, ignoring');
            }
        });
    ...
    _getSelectedAmount() {
        ...
        console.log('[Stripe Badge] Using FALLBACK from paymentContext.amount:', fallbackAmount);
    ...
    async _updateStripeFees() {
        ...
        const paymentFormEl = document.querySelector('#o_payment_form') || this.el;
        const totalInvoiceAmount = this._invoiceTotalAmount || parseFloat(
            paymentFormEl.dataset.amount || baseAmount
        );
    ...
    async _fetchCountryData(provider) {
        ...
        // Fallback: if no delivery country (e.g. invoice without sale order), use invoice customer's country
        if (!deliveryCountryId) {
            const invoiceId = this._extractInvoiceId();
            if (invoiceId) {
                try {
                    const response = await fetch(`/custom/stripe/invoice_partner_country/${invoiceId}`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({}),
                    });
                    const invoiceData = await response.json();
                    deliveryCountryId = invoiceData.result?.country_id || null;
                    this._invoiceTotalAmount = invoiceData.result?.amount_total || null;  // store full invoice total
                } catch (error) {
                    console.error('[Stripe Badge] Failed to fetch invoice partner country:', error);
                }
            }
        }


        console.log("companyCountryId : ",companyCountryId)
    ...
    _extractInvoiceId() {
        if (this.paymentContext.transactionRoute) {
            const matches = this.paymentContext.transactionRoute.match(/\/invoice\/transaction\/(\d+)/);
    ...
    _calculateFees(baseAmount, totalInvoiceAmount, provider, companyCountryId, deliveryCountryId) {
        const isInternational = deliveryCountryId && companyCountryId &&
            deliveryCountryId !== companyCountryId;

        // Use totalInvoiceAmount to determine free-threshold eligibility (mirrors backend check)
        const amountForThreshold = totalInvoiceAmount || baseAmount;

        let totalFixedFees = 0;
        let totalPercentFees = 0;

        if (isInternational) {
            if (!provider.is_free_international || amountForThreshold < (provider.free_international_amount || 0)) {
                // Scale fixed fee proportionally to partial payment amount (mirrors backend logic)
                totalFixedFees = (baseAmount * (provider.fix_international_fees || 0)) / amountForThreshold;
                totalPercentFees = (provider.var_international_fees || 0) * baseAmount / 100;
            }
        } else {
            if (!provider.is_free_domestic || amountForThreshold < (provider.free_domestic_amount || 0)) {
                // Scale fixed fee proportionally to partial payment amount (mirrors backend logic)
                totalFixedFees = (baseAmount * (provider.fix_domestic_fees || 0)) / amountForThreshold;
                totalPercentFees = (provider.var_domestic_fees || 0) * baseAmount / 100;
            }
        }

        let calcFees = Math.round((totalFixedFees + totalPercentFees) * 100) / 100;
