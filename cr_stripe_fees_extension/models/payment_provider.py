...
class PaymentProvider(models.Model):
    ...
    var_international_fees = fields.Float(
        string="Variable International Fees (in percent)"
    )
    is_free_international = fields.Boolean(
        string="Free International Fees if Amount is Above"
    )
        ...
        **kwargs
        ...
        if self.is_extra_fees:
            ...
            if partner.country_id != company_country:
                ...
                total_percent = (
                    (self.var_international_fees * amount) / 100 if amount else 0.0
                )
            else:
                # Domestic fees
                total_fixed = self.fix_domestic_fees
                total_percent = (
                    (self.var_domestic_fees * amount) / 100 if amount else 0.0
                )
