...
class PublicStripeInfo(http.Controller):
    ...
    @http.route(
        ["/custom/stripe/order_shipping_country/<int:order_id>"],
        type="json",
        auth="public",
        csrf=False,
    )
    ...
    @http.route(
        ["/custom/stripe/invoice_partner_country/<int:invoice_id>"],
        type="json",
        auth="public",
        csrf=False,
    )
    def get_invoice_partner_country(self, invoice_id):
        ...
        return {
            ...
            "amount_total": invoice.amount_total,  # full invoice total, never changes after partial payments
        }
