        ...
        if event.get("type") in ("payment_intent.succeeded", "charge.succeeded"):
            ...
            try:
                ...
                for inv in tx.invoice_ids:
                    _logger.info(
                        "Invoice %s before reconcile: state=%s payment_state=%s",
                        inv.id,
                        inv.state,
                        inv.payment_state,
                    )
                    ...
                    inv_receivables = inv.line_ids.filtered(
                    ...
                    pay_receivables = payment.move_id.line_ids.filtered(
                        lambda l: l.account_id.account_type == "asset_receivable"
                        and not l.reconciled
                        and l.parent_state == "posted"
                    )
                    if not pay_receivables:
                        ...
                        pay_receivables = payment.move_id.line_ids.filtered(
                            lambda l: l.account_id == inv_receivables[0].account_id
                            and not l.reconciled
                            and l.parent_state == "posted"
                        )
                    ...
                    for acc in pay_receivables.mapped("account_id"):
                        to_reconcile = (inv_receivables + pay_receivables).filtered(
                        ...
                        try:
                            ...
                            reconciled_any = True
                            _logger.info(
                                "Reconciled invoice %s with payment %s on account %s",
                                inv.id,
                                payment.id,
                                acc.display_name,
                            )
                            ...
                            _logger.info(
                                "Failed to reconcile invoice %s with payment %s on account %s: %s",
                                inv.id,
                                payment.id,
                                acc.display_name,
                                e,
                    ...
                    if not reconciled_any:
                        _logger.info(
                            "Could not reconcile invoice %s with payment %s automatically; please check account/partner/currency settings.",
                            inv.id,
                    ...
                    _logger.info(
                        "Invoice %s after reconcile attempt: state=%s payment_state=%s",
                        inv.id,
                        inv.state,
                        inv.payment_state,
                    )
