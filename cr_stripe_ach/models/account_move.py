...
class AccountMove(models.Model):
    ...
    def action_pay_with_ach(self):
        ...
        if not provider:
            raise UserError(
                "The Stripe payment provider is not configured or disabled."
            )
        ach_method = self.env["payment.method"].search(
            [("code", "=", "ach_direct_debit")], limit=1
        )
        if not ach_method:
        ...
        session = provider._stripe_make_request(
            "checkout/sessions", payload=payload, method="POST"
        )
        if "error" in session:
            raise UserError(
                session["error"].get(
