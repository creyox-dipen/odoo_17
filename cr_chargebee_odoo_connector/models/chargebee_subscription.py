...
class ChargebeeSubscription(models.Model):
    ...
    chargebee_id = fields.Char(
        string="Chargebee Subscription ID", help="ID of the subscription in Chargebee"
    )
    ...
    billing_cycle = fields.Integer(
        string="Billing Cycle", help="Number of billing cycles"
    )
    plan_id = fields.Char(
        string="Plan ID", help="ID of the plan associated with the subscription"
    )
        ...
        try:
            ...
            for sub_data in subscriptions:
                ...
                vals = {
                    ...
                    "start_date": self.convert_timestamp_to_datetime(
                        getattr(subscription, "start_date", None)
                    ),
                    "end_date": self.convert_timestamp_to_datetime(
                        getattr(subscription, "end_date", None)
                    ),
                ...
                existing_subscription = self.search(
                    [("chargebee_id", "=", subscription.id)], limit=1
                )
        ...
        for subscription in self:
            ...
            try:
                chargebee.Subscription.update(
                    subscription.chargebee_id, {"plan_id": new_plan_id}
                )
                subscription.plan_id = new_plan_id
            except Exception as e:
                _logger.error(
                    f"Error upgrading subscription {subscription.chargebee_id}: {e}"
                )
                raise UserError(
                    _(
                        "An error occurred while upgrading the subscription. Please check the logs for details."
                    )
                )
    ...
    def action_downgrade_subscription(self):
        ...
        for subscription in self:
            ...
            try:
                ...
                _logger.error(
                    f"Error downgrading subscription {subscription.chargebee_id}: {e}"
                )
                raise UserError(
                    _(
                        "An error occurred while downgrading the subscription. Please check the logs for details."
                    )
                )
    ...
    def action_cancel_subscription(self):
        ...
        for subscription in self:
            try:
                ...
                _logger.error(
                    f"Error cancelling subscription {subscription.chargebee_id}: {e.json_obj}"
                )
                raise UserError(
                    _("Failed to cancel the subscription. Error: %s")
                    % e.json_obj.get("message", str(e))
                )
            except Exception as e:
                # Handle general errors
                _logger.error(
                    f"Error cancelling subscription {subscription.chargebee_id}: {e}"
                )
                raise UserError(
                    _(
                        "An error occurred while cancelling the subscription. Please check the logs for details."
                    )
                )
