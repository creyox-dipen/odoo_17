...
class ChargebeeItemFamily(models.Model):
    ...
    chargebee_id = fields.Char(
        string="Chargebee Family ID", help="ID of the item family in Chargebee"
    )
    item_ids = fields.One2many(
        comodel_name="product.template", inverse_name="item_family_id", string="Items"
    )
    journal_config_id = fields.Many2one(
        comodel_name="journal.configuration",
    )
    ...
    def sync_item_families(self):
        ...
        try:
            for family_data in families:
                ...
                existing_family = self.search(
                    [("chargebee_id", "=", chargebee_family.id)], limit=1
                )
                if existing_family:
