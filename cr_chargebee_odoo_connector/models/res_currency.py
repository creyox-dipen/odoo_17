...
class ResCurrency(models.Model):
    ...
    def sync_chargebee_currencies(self):
        ...
        if (
            ...
            raise UserError(
                _(
                    "Chargebee configuration is incomplete. Please configure the API key and site name."
                )
            )
        try:
            # Configure Chargebee
            ...
            for currency_data in currencies_response:
                ...
                existing_currency = self.env["res.currency"].search(
                    [("name", "=", cb_currency_code), ("active", "in", [True, False])],
                    limit=1,
                )
                if existing_currency:
                    _logger.info(
                        f"Currency {cb_currency_code} already exists. Skipping creation."
                    )
                    continue
            ...
            _logger.error(
                f"Unexpected error while syncing currencies from Chargebee: {e}"
            )
