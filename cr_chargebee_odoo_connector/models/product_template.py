...
class ProductTemplate(models.Model):
    ...
    chargebee_id = fields.Char(
        string="Chargebee Item ID", help="ID of the item in Chargebee"
    )
        ...
        try:
            ...
            for item_data in items:
                ...
                if item_prices:
                    ...
                    price = (
                        item_price_data.price / 100 if item_price_data.price else 0.0
                    )
    ...
    def create_item_in_chargebee(self):
        ...
        if not self.item_family_id:
            ...
            try:
                ...
                raise ValueError(
                    _("Error creating item family in Chargebee: %s") % str(e)
                )

        if not self.item_family_id.chargebee_id:
            raise ValueError(
                _("The selected item family does not have a Chargebee ID.")
            )
