...
class StripeStatementCollection(StripeController):
        ...
        try:
            ...
            if transaction:
                ...
                if not account_receivable:
                    raise UserError(
                        "Account Receivable not found for the invoice customer!"
                    )
            else:
                # External checkout: no Odoo transaction — leave partner and
                # counterpart empty so reconciliation models can match later.
                invoice_partner = False
                account_receivable = False
                logger.info(
                    "External checkout charge %s — no transaction found, "
                    "counterpart_account_id left empty for reconciliation",
                    stripe_object.get("id"),
                )
    ...
    def _handle_refund(self, journal, stripe_object, provider):
        ...
        try:
            ...
            # take only refunded amount if partial refund
