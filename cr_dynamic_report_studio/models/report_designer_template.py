...
class ReportDesignerTemplate(models.Model):
    ...
    model_id = fields.Many2one('ir.model', string='Bound Model', required=True, ondelete='cascade')
    ...
    is_bound_to_print_menu = fields.Boolean(string='Appears in Print Menu', default=False)
    page_ids = fields.One2many('report.designer.template.page', 'template_id', string='Pages')
    component_ids = fields.One2many('report.designer.component', 'template_id', string='Components')
    report_action_id = fields.Many2one('ir.actions.report', string='Report Action', ondelete='set null')
    ...
    def _compute_last_print_log_id(self):
        for record in self:
            last_log = self.env['report.designer.print.log'].sudo().search(
    ...
    def action_preview_pdf(self, res_ids=None):
        ...
        if not res_ids:
            ...
            if not record:
                ...
                raise UserError(f"No records found for model {self.model_id.name} to generate a preview.")
            res_ids = [record.id]
            ...
            try:
                ...
                if hasattr(e, 'html') and getattr(e, 'html'):
                    ...
                    try:
                        ...
                        node_xml = node_xml[:300] + "..." if len(node_xml) > 300 else node_xml
                    except Exception:
                        pass
    ...
    def _compile_element(self, el, seg_offset_y, margin_top=0, margin_left=0, flow_y_offset=0):
        ...
        if el_rot and el_type != 'watermark':
            outer.append(f"transform: rotate({el_rot}deg); -webkit-transform: rotate({el_rot}deg);")
        ...
        if el_type in ('text', 'field'):
            ...
            css = "; ".join(outer)
            lines.append(f'        <div style="{css}; border: 1pt solid #ccc; background: #fafafa;">{el_type}</div>')
    ...
    def _is_expression_valid(self, model_name, expression):
        ...
        for path in paths:
            ...
            clean_path = '.'.join([p for p in path.split('.') if p not in ('filtered', 'mapped', 'sorted', 'filtered_domain')])
    ...
    def _compile_text_field(self, el, el_type, outer, sty):
        ...
        font_css = f"font-size: {fs}pt; font-family: {ff}; color: {color}; background-color: {bg}; line-height: {lh};"
        if bold:
        ...
        css = "; ".join(outer)
        inner = f"display: table-cell; vertical-align: {valign}; text-align: {ta}; width: 100%; height: 100%; padding: 0; margin: 0; border: none; background: transparent;"

        lines.append(f'        <div style="{css};">')
        
        lines.append(f'          <div style="display: table; table-layout: fixed; width: 100%; height: 100%; padding: 0; margin: 0; border: none; background: transparent; {font_css}">')
        if el_type == 'field':
            ...
            if not field_expr or field_expr == 'field_name':
                lines.append(f'            <div style="{inner}"><span class="text-muted">[Select Field]</span></div>')
            elif not self._is_expression_valid(self.model_name, field_expr):
                lines.append(f'            <div style="{inner}"><span>{self._esc_expr(field_expr)}</span></div>')
            else:
                import re
                if re.match(r'^[a-zA-Z_][a-zA-Z0-9_.]*$', field_expr) and not field_expr.startswith('doc.'):
                    lines.append(f'            <div style="{inner}"><span t-out="doc.{self._esc_expr(field_expr)}"/></div>')
                else:
                    lines.append(f'            <div style="{inner}"><span t-out="{self._esc_expr(field_expr)}"/></div>')
        else:
            text = el.get('content', '')
    ...
    def _process_text_expressions(self, content):
        ...
        def replacer(match):
            ...
            if expr == 'page_number':
            ...
            elif expr == 'total_pages':
                return '<span class="topage"/>'
            elif expr == 'current_date':
                return "<t t-out=\"context_today().strftime('%Y-%m-%d')\"/>"
                ...
                return '<t t-out="base_url"/>'
            import re
            expr_escaped = self._esc_expr(expr)
            if re.match(r'^[a-zA-Z_][a-zA-Z0-9_.]*$', expr) and not expr.startswith('doc.'):
    ...
    def _compile_image(self, el, outer, sty):
        ...
        if img_mode == 'expression' and el.get('expression'):
            ...
            lines.append(f'        <t t-set="_img_val" t-value="doc.{self._esc_expr(field_name)}"/>')
            lines.append(f'        <t t-if="_img_val">')
            lines.append(f'          <div style="{css};">')
            lines.append(f'            <t t-if="isinstance(_img_val, bytes)">')
            lines.append(f'              <img t-att-src="image_data_uri(_img_val)" style="{img_style}" alt=""/>')
            lines.append(f'            </t>')
            lines.append(f'            <t t-else="">')
            lines.append(f'              <span class="text-danger" style="font-size: 10pt; padding: 4pt; display: block;">Invalid Image Field</span>')
            lines.append(f'            </t>')
            lines.append(f'          </div>')
            lines.append(f'        </t>')
            ...
            # Access the resource record through doc.env to get its binary image field
            lines.append(f'          <div style="{css};">')
            lines.append(f'            <t t-set="_res_img" t-value="doc.env[\'report.designer.resource\'].browse({res_id}).image"/>')
            lines.append(f'            <img t-if="_res_img" t-att-src="image_data_uri(_res_img)" style="{img_style}" alt=""/>')
            lines.append(f'          </div>')
        else:
            lines.append(f'          <div style="{css};">')
            lines.append(f'            <div style="width: 100%; height: 100%; background: #eee;"/>')
            lines.append(f'          </div>')
    ...
    def _compile_watermark(self, el, outer, sty):
        ...
        lines.append('          <div style="display: table; table-layout: fixed; width: 100%; height: 100%; padding: 0; margin: 0; border: none; background: transparent;">')
        lines.append('            <div style="display: table-cell; vertical-align: middle; text-align: center; width: 100%; height: 100%; padding: 0; margin: 0; border: none; background: transparent;">')
        ...
        if mode == 'text':
            ...
            text_css = f"color: {color}; font-family: {font_family}, Arial, sans-serif; font-size: {fs_pt}pt; font-weight: bold; white-space: nowrap;"
            lines.append(f'              <div style="{rotate_css} {text_css}">{processed}</div>')
            
        elif mode == 'image':
            img_mode = el.get('imageSourceMode', 'resource')
            img_style = f"max-width: 100%; max-height: 100%; object-fit: contain;"
    ...
    def _compile_barcode(self, el, outer, sty, barcode_type='Code128'):
        ...
        if barcode_type == 'QR':
            ...
            if show_text:
                h = max(10.0, h - 14.0)
            size = min(w, h)
            img_style = f"width: {size}pt; height: {size}pt; display: block; margin: 0 auto;"
    ...
    def _compile_shape(self, el, outer, sty):
        ...
        dash = '5,5' if stroke_style == 'dashed' else ('2,2' if stroke_style == 'dotted' else 'none')

        css = "; ".join(outer)
        lines.append(f'        <div style="{css}">')
        lines.append(f'          <svg width="100%" height="100%" viewBox="0 0 100 100" preserveAspectRatio="none" style="overflow: visible; display: block;">')
        ...
        if shape_type == 'rectangle':
            ...
            lines.append(f'            <rect x="0" y="0" width="100" height="100" rx="{rx}" ry="{rx}" {common}/>')
        elif shape_type == 'ellipse':
            lines.append(f'            <ellipse cx="50" cy="50" rx="50" ry="50" {common}/>')
        elif shape_type == 'triangle':
            lines.append(f'            <polygon points="50,0 100,100 0,100" {common}/>')
        elif shape_type == 'diamond':
            lines.append(f'            <polygon points="50,0 100,50 50,100 0,50" {common}/>')
        elif shape_type == 'polygon':
            lines.append(f'            <polygon points="50,0 95,25 95,75 50,100 5,75 5,25" {common}/>')
        elif shape_type == 'star':
            lines.append(f'            <polygon points="50,0 63,38 100,38 69,59 82,100 50,75 18,100 31,59 0,38 37,38" {common}/>')
    ...
    def _compile_table(self, el, outer, sty):
        ...
        if show_header:
            ...
            lines.append(f'              <tr style="border-bottom: 1pt solid #ced4da; background-color: #f8f9fa; height: {tbl_header_height}pt;">')
            for col in columns:
                ...
                lines.append(f'                <th style="{th_css}">{col.get("header", "")}</th>')
            lines.append('              </tr>')
            lines.append('            </thead>')
        ...
        if data_source:
            ...
            lines.append(f'              <t t-foreach="{self._esc_expr(loop_expr)}" t-as="row">')
            lines.append('                <tr style="border-bottom: 1pt solid #dee2e6;">')
            for col in columns:
                col_w = col.get('widthValue', 50)
                col_w_type = col.get('widthType', 'percent')
                unit = '%' if col_w_type == 'percent' else 'pt'
                ...
                if not c_expr or c_expr == 'field_name':
                    lines.append('                    <span class="text-muted">[Select Field]</span>')
                else:
                    ...
                    if c_expr:
                        clean_expr = c_expr.replace('doc.', 'row.', 1) if c_expr.startswith('doc.') else f"row.{c_expr}"
                    else:
                        clean_expr = ""
                    ...
                    if not is_valid:
                        lines.append(f'                    <span>{clean_expr_esc}</span>')
                    elif c_type == 'image':
                        if clean_expr_esc:
                            lines.append(f'                    <t t-set="_img_val" t-value="{clean_expr_esc}"/>')
                            lines.append(f'                    <t t-if="_img_val">')
                            lines.append(f'                      <t t-if="isinstance(_img_val, bytes)">')
                            lines.append(f'                        <img t-att-src="image_data_uri(_img_val)" style="max-width: 100%; max-height: 48pt; display: block; margin: auto;" alt=""/>')
                            lines.append(f'                      </t>')
                            lines.append(f'                      <t t-else="">')
                            lines.append(f'                        <span class="text-danger" style="font-size: 8pt;">Invalid Image</span>')
                            lines.append(f'                      </t>')
                            lines.append(f'                    </t>')
                    elif c_type == 'barcode':
                        if clean_expr_esc:
                            lines.append(f'                    <div t-if="{clean_expr_esc}" style="text-align: center;">')
                            lines.append(f'                      <img t-attf-src="/report/barcode/?barcode_type=Code128&amp;value={{{{ {clean_expr_esc} }}}}" style="width: 100%; height: 30pt;"/>')
                            lines.append(f'                    </div>')
                    else:
                        if clean_expr_esc:
                            lines.append(f'                    <span t-out="{clean_expr_esc}"/>')
                lines.append('                  </td>')
            lines.append('                </tr>')
            lines.append('              </t>')
        else:
        ...
        if show_footer:
            lines.append('            <tfoot>')
            lines.append('              <tr style="border-top: 1pt solid #ced4da; background-color: #f8f9fa;">')
            for col in columns:
                col_w = col.get('widthValue', 50)
                col_w_type = col.get('widthType', 'percent')
                unit = '%' if col_w_type == 'percent' else 'pt'
                td_css = f"width: {col_w}{unit}; padding: 4pt; border-right: 1pt solid #ced4da; font-size: 11pt; font-weight: bold;"
                lines.append(f'                <td style="{td_css}">{col.get("footer", "")}</td>')
            lines.append('              </tr>')
            lines.append('            </tfoot>')
    ...
    def compile_qweb_arch(self):
        ...
        xml.append(f'<t t-name="cr_dynamic_report_studio.report_designer_template_{self.id}">')
        xml.append('  <main>')
        ...
        for p_idx, page in enumerate(pages):
            ...
            for pb in pagebreaks:
                ...
                segments.append({
                    'offset_y': last_y,
                    'els': [e for e in body_els if e.get('type') != 'pagebreak' and last_y <= e.get('y', 0) < pb_y],
                    'is_last': False,
                })
                last_y = pb_y
            segments.append({
                'offset_y': last_y,
                'els': [e for e in body_els if e.get('type') != 'pagebreak' and e.get('y', 0) >= last_y],
                'is_last': True,
            })
            ...
            for seg in segments:
                ...
                xml.append(f'      <div class="o_designer_report_page" style="{page_css}">')
                ...
                for idx, w_page in enumerate(pages):
                    for el in w_page.get('elements', []):
                        el_type = el.get('type')
                        el_y = el.get('y', 0)
                        
                        is_watermark = (el_type == 'watermark')
                        is_global_header = (idx == 0 and header_enabled and el_y < header_height and el_type != 'watermark')
                        is_global_footer = (idx == 0 and footer_enabled and el_y >= (height - footer_height) and el_type != 'watermark')
                        
                        if is_watermark or is_global_header or is_global_footer:
                            xml.extend(self._compile_element(el, 0))
                ...
                tables = sorted([e for e in seg_body_els if e.get('type') == 'table'], key=lambda e: e.get('y', 0))
                ...
                if seg_body_els:
                    ...
                    if footer_enabled:
                        content_bottom = min(content_bottom, float(height - footer_height))
                    ...
                    xml.append(f'        <div style="position: absolute; left: {content_left}pt; top: {content_top}pt; width: {content_width}pt; height: {content_height}pt; overflow: hidden; margin: 0; padding: 0;">')
                        ...
                        if block['type'] == 'abs':
                            if flow_y_offset > 0:
                                ...
                                if container_height < 0:
                                    container_height = 0
                                xml.append(f'        <div style="position: relative; height: {container_height}pt;">')
                            
                            for bel in block['els']:
                                xml.extend(self._compile_element(bel, seg['offset_y'], margin_top=content_top, margin_left=content_left, flow_y_offset=flow_y_offset))
                            ...
                            if spacer_height > 0:
                                xml.append(f'        <div style="height: {spacer_height}pt; width: 100%;"></div>')
                                
                            xml.extend(self._compile_element(t_el, seg['offset_y'], margin_top=content_top, margin_left=content_left, flow_y_offset=flow_y_offset))
    ...
    def create_or_update_qweb_view(self):
        ...
        if not view:
            view = self.env['ir.ui.view'].search([('key', '=', f'report_designer_template_{self.id}')], limit=1)
        if view:
            view.write({
                'arch': arch,
                'key': view_key,
    ...
    def write(self, vals):
        ...
        trigger_fields = {'template_json', 'paper_width', 'paper_height', 'name',
        ...
        if trigger_fields & set(vals.keys()):
            for record in self:
                ...
                if record.report_action_id or self.env['ir.ui.view'].search([('key', '=', view_key)], limit=1):
    ...
    def action_validate_expression(self, expression, model_name=None):
        ...
        try:
            ...
            elif '=' in expression and '==' not in expression and '!=' not in expression and '<=' not in expression and '>=' not in expression:
                suggestion = "Did you mean to use '==' for comparison?"
            # Common error 3: missing dot prefix for doc
            elif any(part in expression for part in ['partner_id', 'name', 'amount_total']) and not any(expr in expression for expr in ['doc.', 'user.', 'object.']):
        ...
        import re
        invalid_fns = ['upper', 'lower', 'strip', 'split', 'replace', 'find', 'startswith', 'endswith', 'strftime', 'strptime']
            ...
            try:
                ...
                for path in doc_paths:
                    ...
                    for part in parts:
                        ...
                        if part not in current_model._fields:
                            import difflib
                            matches = difflib.get_close_matches(part, list(current_model._fields.keys()), n=1)
                            suggested_part = matches[0] if matches else None
                            
                            wrong_path = "doc." + ".".join(valid_path + [part])
                            if suggested_part:
                                correct_path = "doc." + ".".join(valid_path + [suggested_part])
