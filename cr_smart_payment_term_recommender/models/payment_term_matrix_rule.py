...
class PaymentTermMatrixRule(models.Model):
    ...
    matrix_id = fields.Many2one(comodel_name='payment.term.matrix', string='Matrix', required=True)
    rule_line_ids = fields.One2many(comodel_name="payment.term.matrix.rule.line", inverse_name="rule_id", required=True)
    ...
    def _onchange_category_id(self):
        if len(list(self.matrix_id)):
            ...
            if self.matrix_id.id in defined_matrix_ids:
                raise ValidationError('The rules for this KPI have already been defined !!')