...
class BigQueryScheduler(models.Model):
    ...
    cr_export_model_id = fields.Many2one(
        "ir.model", string="Model To Export", readonly=False, store=True
    )
    ...
    cr_import_model_id = fields.Many2one(
        "ir.model", string="Model To Import", readonly=False, store=True
    )
    ...
    cr_model_name = fields.Char(compute="_compute_cr_model_name", store=False)
    ...
    def _compute_cr_model_name(self):
        for rec in self:
            rec.cr_model_name = (
                rec.cr_export_model_id.model
                if rec.cr_export_model_id
                else rec.cr_import_model_id.model
            )
    ...
    def _compute_available_fields(self):
        for record in self:
            if record.cr_export_model_id:
                fields_domain = [
                    ("model", "=", record.cr_export_model_id.model),
                    ("store", "=", True),
                ]
                ...
                fields_domain = [
                    ("model", "=", record.cr_import_model_id.model),
                    ("store", "=", True),
                ]
                record.available_field_ids = self.env["ir.model.fields"].search(
                    fields_domain
    ...
    def _get_scheduler_config(self):
        ...
        vals = {
            ...
            "model_id": self.env.ref(
                "cr_bigquery_connector.model_cr_big_query_scheduler"
            ).id,
            "state": "code",
    ...
    def run_manually(self):
        record_id = (
            self.env["cr.big.query.scheduler"].sudo().search([("id", "=", self.id)])
        )
        operation_type = record_id.cr_operation
        config = record_id.cr_config_id
        if operation_type == "import":
    ...
    def _is_valid_vat_format(self, vat):
        # Very simple validation: must start with two letters (country code)
        if len(vat) < 3:
            return False
    ...
    def import_records(self, record, config):
        ...
        if not self._check_table_exists(client, table_id):
            raise ValidationError(
                _("The selected table '%s' does not exist in BigQuery.") % model_name
            )
        ...
        if not field_names:
            raise ValidationError(
                _(
                    "No matching fields found between Odoo model '%s' and BigQuery table '%s'"
                )
                % (model_name, table_id)
            )
            ...
            try:
                ...
                for condition in parsed_domain:
                    ...
                    if field not in field_names:
                        continue
                    field_type = (
                        model_fields.get(field).type
                        if model_fields.get(field)
                        else "char"
                    )
                    ...
                    if value in [False, "False", "false", None, "None", ""]:
                        if operator == "=":
                            ...
                            raise ValidationError(
                                _(
                                    "Unsupported operator '%s' for NULL-like value on field '%s'"
                                )
                                % (operator, field)
                            )
                        continue
                ...
                if self._is_value_valid(field_value):
                    try:
                        ...
                        if field_name == "vat" and field_value:
                            if not self._is_valid_vat_format(field_value):
                                continue
                        ...
                        print(
                            f"[BQUERY IMPORT] Failed to process field {field_name}: {str(e)}"
                        )
