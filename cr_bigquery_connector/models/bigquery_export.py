...
class BigQueryExport(models.Model):
    ...
    def _is_valid_vat_format(self, vat):
        """Basic GST/VAT sanity validation to match Odoo 17 requirement."""
        print("checking vat number")
        if not vat or len(vat) < 4:
            return False

        # Allow GST-like formats (India)
        if len(vat) == 15 and vat[0:2].isdigit():
            return True

        # Allow only CC## (Odoo's default simple VAT pattern)
        if len(vat) >= 4 and vat[0:2].isalpha() and vat[2:].isalnum():
            return True
            ...
            for row in rows:
                ...
                if record_id is None:
                    ...
                    for field_name in query_fields:
                        ...
                        if field_name == "vat":
                            if not field_value or not self._is_valid_vat_format(
                                field_value
                            ):
                                continue
